...
\begin{abstract}
El presente proyecto describe la base teórica e implementación de un programa computacional para el procesamiento y análisis de imágenes satelitales utilizando la Transformada Rápida de Fourier (FFT) en dos dimensiones. El objetivo principal consiste en aislar y resaltar las características estructurales de alta frecuencia, tales como frentes nubosos y contornos geográficos, mediante la atenuación de las regiones de baja frecuencia. Para lograr esto, se aplicó un Filtro Pasa-Altas Ideal (IHPF) en el dominio de la frecuencia, evaluando la respuesta visual y numérica del sistema ante diferentes radios de corte. Adicionalmente, se incorporó un análisis cuantitativo fundamentado en el Teorema de Parseval para medir el porcentaje de energía espectral conservada y eliminada tras el filtrado. Los resultados obtenidos confirman la alta eficiencia de la FFT para el procesamiento digital de señales y validan la utilidad del método para extraer detalles topológicos, aunque también exponen las limitaciones de este tipo de filtro.
\end{abstract}

\vspace{0.5em}
\begin{IEEEkeywords}
La Transformada Discreta de Fourier Bidimensional
(DFT 2D), Transformada Rápida de Fourier (FFT), Procesamiento de imágenes, Conversión escala grises, Filtro pasa-altas, Dominio de la frecuencia, Teorema de Parseval, Efecto de Gibbs, Google Colaboratory, Python
\end{IEEEkeywords}
...
Dado que las computadoras operan exclusivamente con información discreta y finita, la herramienta fundamental para el procesamiento digital de señales es la Transformada Discreta de Fourier (DFT).La Transformada Discreta de Fourier (DFT) es el instrumento central del presente trabajo. Antes de tratar las imágenes, presenta su la versión más simple de ellas. Es decir, transforma una secuencia finita de $N$ muestras $x[n]$, en un nuevo conjunto de $N$ coeficientes complejos $X[k]$, donde cada coeficiente está asociado a una frecuencia específica \cite{kulkarni, tan2020}:
...
De acuerdo al principio de expansión, el núcleo exponencial de esta transformación se fundamenta en la identidad de Euler \cite{castillo2018filtrado}:
\begin{equation}
e^{-i \theta} = \cos(\theta) - i\sin(\theta)
\end{equation}
lo que explica por qué el operador proyecta la intensidad real de los píxeles hacia un espacio de coeficientes complejos $\mathbb{C}^{M \times N}$, estructurados por una componente real $\operatorname{Re}(F(u,v))$ y una imaginaria $\operatorname{Im}(F(u,v))$ \cite{gonzalez2018}.
...
El filtrado digital en el dominio de la frecuencia consiste en la modificación de los componentes espectrales de una imagen antes de restituirla al dominio espacial. Para una imagen digital de entrada $f(x,y)$ de dimensiones $P \times Q$ píxeles (generalmente expandida mediante técnicas de relleno o \textit{padding} para evitar artefactos de convolución circular), la ecuación fundamental de filtrado se define mediante la expresión \cite{gonzalez2018}:
\begin{equation}
g(x,y) = \operatorname{Real}\left\{ \mathcal{F}^{-1} [H(u,v) \cdot F(u,v)] \right\}
\end{equation}

donde $\mathcal{F}^{-1}$ representa el operador de la Transformada Discreta de Fourier Inversa (IDFT 2D), $F(u,v)$ es la DFT de la imagen original, $H(u,v)$ denota la función de transferencia del filtro, y $g(x,y)$ es la imagen resultante procesada en el espacio de coordenadas reales. El producto $G(u,v) = H(u,v) \cdot F(u,v)$ opera estrictamente mediante una multiplicación elemento por elemento (producto de Hadamard) sobre arreglos bidimensionales de tamaño uniforme \cite{gonzalez2018}.

\subsubsection{Teorema de Convolución}

El fundamento teórico que justifica este procedimiento es el \textit{teorema de convolución}. Este establece que la convolución de dos señales en el dominio espacial es equivalente a la multiplicación de sus
transformadas de Fourier en el dominio de frecuencias \cite{oppenheim2010}:

\begin{equation}
    I(x,yx) * h(x,y) \;\longleftrightarrow\; F(u,v) \cdot H(u,v)
...
Un filtro pasa-altas tiene por objetivo suprimir las componentes de baja frecuencia del espectro y conservar únicamente las de alta frecuencia. En el Filtro Pasa-Altas Ideal (\textit{Ideal High-Pass Filter}, IHPF), las funciones de transferencia son simétricas respecto a su origen geométrico. Para cumplir con esta condición, el espectro $F(u,v)$ se centraliza computacionalmente desplazando el origen al centro de la matriz $\left(\frac{P}{2}, \frac{Q}{2}\right)$ \cite{gonzalez2018}.

El comportamiento numérico del filtro está condicionado por la distancia euclidiana medida en línea recta desde cada componente de frecuencia $(u,v)$ hacia dicho centro espectral, expresada analíticamente como:
\begin{equation}
